...
router.put("/coins1", async (req, res) => {
  try {
    if (!req.session.logged_in) {
      ...
      if (watcher.videoon == 2) {
        //check that the user is not already watching the video in another tab
        res.status(400);
      } else {
        ...
        await user.update(
          {
            //create temp coupon and set the status to "watching"
            tempCoupon: RANDO,
            videoon: 2,
          },
          {
        ...
        setTimeout(() => {
          //after ten seconds set status to "not watching"
          user.update(
            {
              videoon: 1,
            },
        ...
        setTimeout(() => {
          //after 15 seconds return the temporary coupon to get coins
